 ...
 */
export const studentsToAlgolia = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    console.log(
      `✅ Updated google_id, facebook_id, apple_id in Firestore for student ${context.params.id}`
    );
  });

  export const tutorsToAlgolia = functions.firestore
  .document("tutors/{id}")
  .onWrite(async (change, context) => {
    const after = change.after;
    if (!after.exists) {
